...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.tokenStorage.getToken()) {
      this.getUserCountries(this.tokenStorage.getUser());
    }else{
  ...
  getUserCountries(userid : string) {
    this.userService.getUserCountries(userid).subscribe({
  ...
  getCountriesInfo(response: any): void {
    ...
    this.countries.forEach(item => {
      this.getWeatherDetails(item.country);
    })
  };
  getWeatherDetails(country: string) {
    var body = JSON.stringify({
      "country": country,
    });
    this.userService.getWeather(body).subscribe({
      next: this.returnWeatherInfos.bind(this),
      error: this.handleError.bind(this)
    });
  }
  returnWeatherInfos(response: any): void {
    this.weatherDetails.push(response);
